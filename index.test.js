...
function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  ...
  for (let i = 0; i < plainTextPositions.length; i++) {
    const { start: s, end: e } = plainTextPositions[i];
    const open = "<mark>";
    const close = "</mark>";
    let lo = content.indexOf(plainText.substring(s, e), s - 1);
    let hi = lo + (e - s);
    content =
      content.substring(0, lo) +
      open +
      content.substring(lo, hi) +
      close +
...
testcases.map((val, i) => {
  test(`For case ${i + 1}:- `, () => {
    expect(
      highlightHTMLContent(htmlContent, plainText, val.plainTextPositions)
    ).toBe(val.result);
  });
});
